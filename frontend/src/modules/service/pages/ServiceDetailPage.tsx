    ...
    <div className="panel">
      ...
      {query.data ? (
        <>
          ...
          <p>
            <strong>Status:</strong> {query.data.status}
          </p>
          <p>
            <strong>Updated:</strong> {query.data.updatedAt ? new Date(query.data.updatedAt).toLocaleString() : "n/a"}
          </p>
          <p>
            <Link to={`/dms/service/${query.data.id}/edit`}>Edit</Link>
          </p>
        </>
      ) : null}
      {query.error ? <ErrorPanel error={query.error} title="Service record unavailable" /> : null}
    </div>
  );
}

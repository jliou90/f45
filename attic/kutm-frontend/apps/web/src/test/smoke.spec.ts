...
test("login launcher picker drawer conflict flow", async ({ page }) => {
  ...
  await page.getByTestId("resolve-reload").dispatchEvent("click");
  await expect(page.getByTestId("conflict-dialog")).toBeHidden();

  await page.getByTestId("save-customer").click({ force: true });
  await expect(page.getByTestId("customer-name-input")).toBeHidden();
});

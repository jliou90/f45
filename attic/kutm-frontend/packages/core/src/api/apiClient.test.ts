...
describe("apiClient", () => {
  ...
  it("attaches Authorization and X-Tenant-Id", async () => {
    ...
    await apiClient.request<{ ok: boolean }>({ method: "GET", path: "/me" });
  ...
  it("converts 412 into ConflictError", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(JSON.stringify({ error: { code: "conflict", message: "Precondition failed" } }), {
        status: 412,
        headers: { "content-type": "application/json" }
      })
    );
    ...
    await expect(
      apiClient.request({ method: "PATCH", path: "/customers/c1", body: { name: "x" }, ifMatch: '"e1"' })
    ).rejects.toMatchObject({ kind: "conflict_error", entityType: "customer", entityId: "c1" });
  });
});

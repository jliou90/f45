...
function requireAuthAndTenant(req: Request) {
  ...
  if (!tenant) {
    return HttpResponse.json(
      { error: { code: "validation_error", message: "Missing X-Tenant-Id" } },
      { status: 400 }
    );
  }
  return null;
...
export const handlers = [
  http.post("/api/auth/login", () =>
    HttpResponse.json({ accessToken: "access_1", refreshToken: "refresh_1", expiresInSec: 900 })
  ),
  ...
  http.patch("/api/customers/:id", async ({ request, params }) => {
    ...
    const body = (await request.json()) as Partial<typeof fixtures.customer>;
    const next = {
      ...fixtures.customer,
      ...body,
